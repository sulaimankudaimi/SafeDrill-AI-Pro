...
with st.sidebar:
    ...
    st.markdown("### 🛡️ SafeDrill AI Pro")
    st.markdown(f"**Lead Developer:**\nEng. Sulaiman Kudaimi")
    st.markdown("---")
    # إضافة حقل تكلفة الـ NPT لإبهار المسؤول
...
st.title("🛡️ Operational Drilling Safety & Advisor")
...
if uploaded_file is not None:
    ...
    try:
        ...
        # --- الإضافة رقم 2: AI Action Advisor ---
        st.markdown("### 🤖 AI Technical Advisor")
        if risk_percent > 5:
            st.error(f"🚨 CRITICAL RISK: {risk_percent:.1f}%")
            st.warning("""
            **Recommended Actions:**
            ...
            st.success("✅ Operational Status: STABLE")
        ...
        # --- الإضافة رقم 3: ميزة التقرير الفوري ---
        st.markdown("---")
        if st.button("📝 Generate Incident Summary Report"):
            ...
            st.download_button("📥 Download Report as TXT", report_text, file_name="Drilling_Report.txt")

    except Exception as e:
        st.error(f"Data processing error: {e}")

st.markdown("---")
st.caption("© 2026 | Eng. Sulaiman Kudaimi | SafeDrill AI Pro v2.0")